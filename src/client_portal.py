...
def handle_client(stream, client, replica_stream, cache):
    ...
    while True:
        ...
        response = None
        if (request.req == RequestType.CreateTask or request.req == RequestType.UpdateTask or request.req == RequestType.ListTasks 
            or request.req == RequestType.DeleteTask or request.req == RequestType.DeleteAllTasks):
            
            with lock:
                if request.req == RequestType.ListTasks:
